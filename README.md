...
2. Deployment triggers (branch tracking) are per-service, per-environment rows — not a single project-wide setting. For each of `travelplan-web`/`travelplan-worker`, add a trigger with `branch: develop` pointed at the new `staging` environment (Settings → each service → Triggers in the dashboard; via API it's the `deploymentTriggerCreate` GraphQL mutation with the existing production trigger's `repository`/`provider` copied over). This mirrors production's `main` trigger without touching it.
