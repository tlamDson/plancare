...
**Nợ kỹ thuật đã xác minh** (không tự sửa trừ khi được giao — xem `.claude/rules/tech-defaults.md` để có danh sách đầy đủ, gồm cả các bug phát hiện khi dựng hạ tầng test): `trip.controller.ts` 882 dòng vi phạm Rule of 200 với 14 chỗ `as any`; `validation.service.ts`'s `PlaceCache` có bug cache-key mù destination + mất field `photoUrl`/`openingHoursArray` khi cache hit (cache **có** nằm đúng chỗ ở caller layer, không phải "thiếu cache" như ghi chú cũ ở đây từng nói). (`generalLimiter`/auth cho `/api/dev/scrape-insights` đã fix ở PR `fix/general-limiter-and-dev-route-auth`; Stripe eager-init/`CLERK_PUBLISHABLE_KEY` thiếu khai báo đã fix ở PR `fix/boot-env-resilience`; `/api/dev/*`+`/api/docs` gate ở mức mount + Pino redaction + `service: "worker"` tag đã fix ở PR `fix/security-gates-and-log-hygiene`.)
