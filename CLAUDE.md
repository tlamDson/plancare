...
**Error tracking & backup đã dựng xong khung, chờ credentials** (đợt PR #38-42, xem `.claude/rules/tech-defaults.md` mục "Error tracking & Backup" để biết chi tiết): Sentry (`@sentry/node`/`@sentry/react`) wire sẵn cho api/worker/web, tag `service` khớp với Pino, nhưng **chưa hoạt động thật** — `SENTRY_DSN`/`VITE_SENTRY_DSN` chưa có giá trị ở bất kỳ môi trường nào, cần tạo project Sentry rồi set trên Railway/Vercel. Backup MongoDB hằng ngày (`backend/scripts/backup-mongo.sh` + `.github/workflows/backup-mongo.yml`) cũng **chưa chạy thật** — thiếu 4 GitHub secret `BACKUP_S3_*`/`BACKUP_AWS_*_KEY` (soft-skip, không đỏ CI), cần tạo bucket R2/S3 trước. Runbook restore ở `backend/scripts/BACKUP.md`.
